...
export default class extends Controller {
    ...
    disconnect() {
        ...
        if (this.autoDismissTimers) {
            Object.values(this.autoDismissTimers).forEach((timer) =>
                clearTimeout(timer)
            );
    ...
    showToast(message, options = {}) {
        const detail = {
            ...
            position:
                options.position ||
                window.currentToastPosition ||
                this.positionValue, // Use stored position
            html: options.html || "",
    ...
    beforeCache() {
        ...
        window.currentToastPosition =
            this.element.dataset.toastPositionValue || "bottom-right";
    }
    ...
    createToastElement(toast) {
        ...
        if (!toast.html && (toast.action || toast.secondaryAction)) {
            requestAnimationFrame(() => {
                if (toast.action) {
                    const primaryBtn = span.querySelector(
                ...
                if (toast.secondaryAction) {
                    const secondaryBtn = span.querySelector(
        ...
        closeBtn.className = `absolute right-0 p-1.5 mr-2.5 text-neutral-400 duration-100 ease-in-out rounded-full cursor-pointer hover:bg-neutral-50 dark:hover:bg-neutral-700 hover:text-neutral-500 dark:hover:text-neutral-300 ${
            !toast.description && !toast.html && !hasActions
    ...
    removeToast(id, isOverflow = false) {
        ...
        setTimeout(() => {
            ...
            if (this.toasts.length >= this.limitValue) {
                ...
                if (
                    newlyVisibleToast &&
                    !this.autoDismissTimers[newlyVisibleToast.id]
                ) {
        ...
        requestAnimationFrame(() => {
            ...
            this.toasts.forEach((toast, index) => {
                ...
                for (let i = 0; i < index; i++) {
                    ...
                    const heightInfo = this.heights.find(
                        (h) => h.toastId === this.toasts[i].id
                    );
                    if (heightInfo) {
                ...
                const heightInfo = this.heights.find(
                    (h) => h.toastId === toast.id
                );
                if (heightInfo) {
                    toastEl.style.setProperty(
                        "--initial-height",
                        `${heightInfo.height}px`
                    );
                }
    ...
    updateContainerHeight() {
        ...
        if (this.expanded) {
            ...
            const frontHeight = frontToast
                ? this.heights.find((h) => h.toastId === frontToast.id)
                      ?.height || 0
                : 0;
            const peekAmount = 24;
    ...
    get positionClasses() {
        ...
        return positions[this.positionValue] || positions["bottom-right"];
    }
}
